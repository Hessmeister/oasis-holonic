...
class KernelDiagram {
  ...
  draw(t) {
    ...
    for (let i = rings.length - 1; i >= 1; i--) {
      ...
      if (ring.dash[0] > 0) { ctx.setLineDash(ring.dash); ctx.lineDashOffset = loop ? -(t * 0.006 * ring.dir) : 0; }
    ...
    if (loop) {
      this.dots.forEach(dot => {
        ...
        grad.addColorStop(0, 'rgba(254,218,179,0.4)'); grad.addColorStop(1, 'rgba(254,218,179,0)');
        ctx.fillStyle = grad;
        ctx.fillRect(dx - dot.size * 4, dy - dot.size * 4, dot.size * 8, dot.size * 8);
        ctx.beginPath(); ctx.arc(dx, dy, dot.size, 0, Math.PI * 2);
        ctx.fillStyle = `rgba(254,218,179,${0.65 + Math.sin(t * 0.002 + dot.angle) * 0.15})`; ctx.fill();
      });
    }
    ...
    if (e1 > 0.65) {
      ...
      inv.forEach(({ text, a }) => {
        ...
        ctx.save(); ctx.translate(lx, ly); ctx.scale(scale, scale);
        if (lit > 0.05) { ctx.shadowColor = `rgba(255,255,255,${0.7 * lit})`; ctx.shadowBlur = 14 * lit; }
    ...
    if (e2 > 0.65) {
      ...
      rules.forEach(({ text, a }) => {
        ...
        ctx.save(); ctx.translate(lx, ly); ctx.scale(scale, scale);
        if (lit > 0.05) { ctx.shadowColor = `rgba(255,255,255,${0.5 * lit})`; ctx.shadowBlur = 12 * lit; }
    ...
    if (e3 > 0.6) {
      ...
      items.forEach((label, i) => {
        ...
        const scale = 1 + lit * 0.08;
        ctx.save(); ctx.translate(lx, ly); ctx.scale(scale, scale);
        if (lit > 0.05) { ctx.shadowColor = `rgba(255,255,255,${0.5 * lit})`; ctx.shadowBlur = 10 * lit; }
        ctx.fillStyle = `rgba(254,218,179,${alpha})`;
    ...
    if (e4 > 0.55) {
      ...
      envs.forEach((label, i) => {
        ...
        const pw = tw + 14; const ph = fontSize + 8;
        ...
        if (lit > 0.05) { ctx.shadowColor = `rgba(255,255,255,${0.4 * lit})`; ctx.shadowBlur = 8 * lit; }
        ctx.fillStyle = `rgba(255,254,223,${textAlpha * lp})`;
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(label, nx, ny + 0.5);
    ...
    if (e0 > 0) {
      ...
      grad.addColorStop(0, `rgba(255,55,0,${0.95 * e0})`); grad.addColorStop(1, `rgba(255,55,0,${0.98 * e0})`);
      ...
      if (e0 > 0.4) {
        ...
        ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText('HOLON', cx, cy);
      }
    }
    ...
    if (ec > 0.2) {
      ...
      lines.forEach((line, i) => {
        ...
        ctx.textAlign = 'center'; ctx.textBaseline = 'top'; ctx.fillText(line.t, cx, captionY + i * lineH);
      });
    }
  }

  ringP(rp, ring) { const delay = ring * 0.12; return Math.max(0, Math.min(1, (rp - delay) / 0.3)); }
  ...
  drawPill(ctx, x, y, w, h, r, fill, stroke) {
    ...
    ctx.fillStyle = fill; ctx.fill(); ctx.strokeStyle = stroke; ctx.lineWidth = 0.6; ctx.stroke();
  }

  loop = (ts) => { if (!this.running) return; this.draw(ts); requestAnimationFrame(this.loop); }
...
function _init_kernel() {
  ...
  if (prefersReducedMotion) {
    kernel.revealProgress = 1; kernel.revealed = true; kernel.draw(0);
  } else {
