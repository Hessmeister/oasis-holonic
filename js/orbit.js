...
class OrbitAnimation {
  constructor(canvas) {
    ...
    this.bodies = [
      { label: 'Star',   tier: 'I',   orbit: 0,    angle: 0,   speed: 0,       size: 24, shape: 'circle' },
      { label: 'Planet', tier: 'II',  orbit: 0.30, angle: 0.8, speed: 0.00025, size: 12, shape: 'circle' },
      { label: 'Moon',   tier: 'III', orbit: 0.10, angle: 2.5, speed: 0.0012,  size: 7,  shape: 'circle', parent: 1 },
      { label: 'Zome',   tier: 'IV',  orbit: 0.05, angle: 4.8, speed: 0.003,   size: 4,  shape: 'diamond', parent: 2 },
    ];
  ...
  resize() {
    ...
    const size = Math.min(parent.offsetWidth, 680);
    this.size = size;
  ...
  getBodyPos(body, t) {
    ...
    if (body.parent !== undefined) {
      ...
      parentX = pp.x; parentY = pp.y;
    }
    const orbitR = body.orbit * this.size;
    const angle = body.angle + t * body.speed;
    return { x: parentX + Math.cos(angle) * orbitR, y: parentY + Math.sin(angle) * orbitR };
  }

  drawShape(ctx, x, y, shape, size) {
    ctx.beginPath();
    if (shape === 'diamond') {
      ctx.moveTo(x, y - size); ctx.lineTo(x + size * 0.7, y); ctx.lineTo(x, y + size); ctx.lineTo(x - size * 0.7, y); ctx.closePath();
    } else {
  ...
  draw(t) {
    ...
    if (rp > 0.4) {
      ...
      for (let i = 1; i < this.bodies.length; i++) {
        ...
        ctx.beginPath(); ctx.moveTo(parentX, parentY); ctx.lineTo(pos.x, pos.y);
        ctx.strokeStyle = `rgba(254,218,179,${lineAlpha})`; ctx.lineWidth = 0.5; ctx.stroke();
      }
    }
    ...
    this.bodies.forEach((body, i) => {
      ...
      if (i === 0) {
        ...
        const coreGrad = ctx.createRadialGradient(pos.x - s * 0.15, pos.y - s * 0.15, 0, pos.x, pos.y, s);
        coreGrad.addColorStop(0, `rgba(255,80,20,${0.95 * bodyP})`);
        coreGrad.addColorStop(1, `rgba(255,55,0,${0.98 * bodyP})`);
        ctx.beginPath(); ctx.arc(pos.x, pos.y, s, 0, Math.PI * 2); ctx.fillStyle = coreGrad; ctx.fill();
        ctx.beginPath(); ctx.arc(pos.x, pos.y, s, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(255,120,60,${0.4 * bodyP})`; ctx.lineWidth = 1; ctx.stroke();
      } else {
        this.drawShape(ctx, pos.x, pos.y, body.shape, s);
        ctx.fillStyle = `rgba(255,55,0,${0.12 * bodyP})`; ctx.fill();
        this.drawShape(ctx, pos.x, pos.y, body.shape, s);
        ctx.strokeStyle = `rgba(254,218,179,${0.65 * bodyP})`; ctx.lineWidth = 1.5; ctx.stroke();
        if (body.size > 5) {
...
function _init_orbit() {
  ...
  if (prefersReducedMotion) {
    orbit.revealProgress = 1; orbit.revealed = true; orbit.draw(0);
  } else {
    const revealObs = new IntersectionObserver((entries) => {
      entries.forEach(e => {
        if (e.isIntersecting && !orbit.revealed) {
          orbit.animateReveal();
        }
      });
    }, { threshold: 0.2 });
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', _init_orbit);
} else {
  _init_orbit();
}
